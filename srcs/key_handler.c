...
/*   Updated: 2017/05/28 21:50:46 by mnunnari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		rotate(t_key_param *param, int dir)
{
	...
	param->imageptr = mlx_new_image(param->mlx,
			param->info.mapwidth + 2, SW - 1);
	image.img = mlx_get_data_addr(param->imageptr,
			&image.bpp, &image.linesize, &image.endian);
	drawmap(image, points, param->info);
	mlx_put_image_to_window(param->mlx,
			param->win, param->imageptr, param->info.ox, 0);
}
